...
# 任务
对给出的每篇论文，输出中文精读内容，字段如下（公式用 $...$ LaTeX 表示）：
- cn_title：中文译题（英文技术缩写保留原样）
- one_liner：一句话简介（30–50 字），概括这篇论文做了什么、得到什么最重要的结果；以论文为主语，不简单重复标题；公式用 Unicode 纯文本
- cn_summary：速览，250–350 字，分两段：
  第一段（2–3 句）背景与动机：这个领域已知什么、还缺什么、为什么这个问题值得做；
  第二段（3–4 句）内容总结：方法 → 关键结果，必须保留摘要中的具体数字、置信度、样本量、对象名称等硬信息
- cn_review：评价，200–250 字，分三段，分别以【创新点】【局限性】【对 Rao 的意义】开头：
  【创新点】1–2 句：与已有工作相比新在哪（具体说明，引用论文关键结果）；
  【局限性】1–2 句：方法或假设最主要的弱点/适用范围（如只查了某个扇区/依赖特殊背景/只是必要条件）；摘要未体现就明说；
  【对 Rao 的意义】1–2 句：结合画像说明对 Rao 有什么用——和哪条研究线/课题卡相关、能借用什么；禁止编造
- learn：「Rao 可以学到/了解到什么」，结合画像中的知识状态与当前学习模块，指出可借用的工具、恒等式、反例或需警惕的约定差异（公式用 Unicode 纯文本）
...
# 输出（严格 JSON）
{{"papers": [{{"id": "arXiv号", "cn_title": "...", "one_liner": "...", "cn_summary": "...", "cn_review": "...", "learn": "...", "topics": "..."}}]}}"""
    ...
    if core:
        ...
        for c in core:
            ...
            papers.append({
                ...
                "cn_summary": d.get("cn_summary", ""),
                "cn_review": d.get("cn_review", ""),
